...
// 🆕 Task 6: 注目度スコア（いいね + 投票の合計）
// 投票は少し軽め（×0.5）にして、いいねを重視した指標にする
const attentionScore = (post) => {
  const likes = post.likes || 0;
  const votes = totalVoteCount(post.votes);
  return likes * 2 + votes;
};

// 🆕 Task 6: 投稿主だけが使うランキング計算
// 指定された投稿が、全投稿の中で注目度何位かを返す
// 例: { rank: 3, total: 6, percentile: 50 }
...
// 🆕 Task 6: 並び順の定義
const SORT_ORDERS = [
  { id:"newest",    label:"新着",      icon:"🆕", grad:"linear-gradient(135deg,#60a5fa,#818cf8)" },
  { id:"attention", label:"注目",      icon:"🔥", grad:"linear-gradient(135deg,#f43f5e,#f97316)" },
  { id:"shuffle",   label:"シャッフル", icon:"🔀", grad:"linear-gradient(135deg,#c084fc,#ec4899)" },
];

// 並び順を投稿配列に適用する
// shuffleSeed は同じ値なら同じ順番になる（再レンダリングで順番がブレないように）
...
export default function HyokaApp() {
  ...
  // 🆕 エラー時もちゃんとフィードバックが出るように改善
  const handleImageUpload = async (e) => {
    const file = e.target.files && e.target.files[0];
    if (!file) return;
    // 入力リセット（同じファイルを選び直せるように）
    e.target.value = "";
...

{
  ...
  "personalColor": {
    ...
    // 【段階2】 Anthropic APIを直接呼ぶ（開発者がlocalhostなどで試す想定）
    ...
    // 【段階3】 どこでも必ず動くデモ結果
    // AIの代わりに、見た目の多様性のためサンプルをランダムにする
  ...
  // シャッフルボタンが押されたときはシードを更新して、本当に順番が変わるようにする
    ...
    <div style={{minHeight:"100vh",
      ...
      {mode==="result" && myPost && (
        <div style={{position:"relative",zIndex:1,padding:"36px 24px 60px",display:"flex",flexDirection:"column",gap:20}}>
          ...
          {/* 🆕 Task 6: みんなの中でのあなたの位置（投稿主だけが見える） */}
      ...
      {SORT_ORDERS.map(order => {
        ...
        return (
          <button
            ...
            style={{
              flex:1, padding:"7px 4px", borderRadius:11, border:"none",
              background: active ? order.grad : "transparent",
              color: active ? "#fff" : "rgba(255,255,255,0.45)",
...
// ═══════════════════════════════════════════════════════════════
// 🆕 Task 6: MyPostRanking — 投稿主だけが見える順位バッジ
//   注目順ランキングでのあなたの位置をキラキラしたカードで表示する
// ═══════════════════════════════════════════════════════════════
    ...
    <div style={{
      ...
      <div style={{padding:"10px 14px",
        ...
        <div style={{fontSize:12, fontWeight:800, color:"rgba(255,255,255,0.85)",
          display:"flex", alignItems:"center", justifyContent:"space-between"}}>
          <span>📊 みんなの中でのあなたの位置</span>
          <span style={{fontSize:10, color:"rgba(255,255,255,0.45)", fontWeight:600}}>🔒 あなただけに表示</span>
        </div>
      </div>
